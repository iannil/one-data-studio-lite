...
CREATE INDEX idx_sensor_readings_is_anomaly ON sensor_readings(is_anomaly) WHERE is_anomaly = TRUE;
...
CREATE INDEX idx_device_events_acknowledged ON device_events(acknowledged) WHERE acknowledged = FALSE;
...
CREATE INDEX idx_maintenance_logs_maintenance_type ON maintenance_logs(maintenance_type);
CREATE INDEX idx_maintenance_logs_status ON maintenance_logs(status);
CREATE INDEX idx_maintenance_logs_scheduled_at ON maintenance_logs(scheduled_at);
CREATE INDEX idx_maintenance_logs_technician_id ON maintenance_logs(technician_id);

-- ============================================
-- Comments for documentation
-- ============================================
COMMENT ON DATABASE iot_db IS 'IoT platform database for device management, sensor data, and monitoring';
COMMENT ON TABLE device_types IS 'Device type catalog with specifications';
COMMENT ON TABLE devices IS 'Device inventory with status and location';
COMMENT ON TABLE sensors IS 'Sensor definitions and configurations';
COMMENT ON TABLE sensor_readings IS 'Time-series sensor data with partitioning';
COMMENT ON TABLE device_events IS 'Device lifecycle and operational events';
COMMENT ON TABLE alerts IS 'Alert and notification records';
COMMENT ON TABLE maintenance_logs IS 'Device maintenance and service records';
